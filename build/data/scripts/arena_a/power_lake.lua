...
function onUpdate()
	...
	for i,v in ipairs(objects_in_radius2) do
		if string.sub(GetObjectName(v:GetObject()), 1, 9) == "bilbpower" then
			bilb = FindObject(GetObjectName(v:GetObject()))
			obj = bilb
			for j,x in ipairs(objects_in_radius) do			
				if string.sub(GetObjectName(x:GetObject()), 1, 9) == "bilbpower" then
		...
		if obj:IsValid() and string.sub(GetObjectName(obj:GetObject()), 1, 9) == "bilbpower"   and delete then
			delete2 = true
