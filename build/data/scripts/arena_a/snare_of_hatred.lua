...
function onUpdate()
			...
			if SHOWN then
				...
				if not done[i] then
					...
					if IsDerivedOrExactClass(v:GetObject(), "Player") then
						SetWorldPosition(bilboard[i]:GetObject(), (GetWorldPosition(v:GetObject())+mkVec3.new(0, 2.5 ,0)))		
					end
					if IsDerivedOrExactClass(v:GetObject(), "ActorAI") then
						SetWorldPosition(bilboard[i]:GetObject(), (GetWorldPosition(v:GetObject())+mkVec3.new(0, 5.5, 0)))		
					end
					done[i] = true
				end
					if IsDerivedOrExactClass(v:GetObject(), "Player") then
	...
	if HIDING then
		if time <= 2.0 then
			SetWorldPosition(this, pos - mkVec3.new(0, 4 * GetTimeDelta(), 0))
		else
			HIDING = false
			HIDDEN = true
			stop_time = false
