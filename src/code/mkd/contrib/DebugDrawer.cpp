...
{  
    MK_ASSERT( ms_Singleton );  return ( *ms_Singleton );  
}
...
void DebugDrawer::buildArrow(const Ogre::Vector3& start,
                            const Ogre::Vector3& end,
                            const Ogre::ColourValue& colour,
                            float alpha)
{
    const float arrow_length = start.distance(end);
    Vector3 dir = end - start;
    Vector3 right;
    
    // Special case for vertical arrows
    if (fabsf(dir.dotProduct(Vector3::UNIT_Y)) >= 0.99f)
        right = Vector3::UNIT_X;
    else
        right = dir.crossProduct(Vector3::UNIT_Y);
    
    right.normalise();
    dir.normalise();

    const float head_part = 0.25f; // arrowhead length will be 0.1 of total len
    const float horizontal_part = 0.5f;
    const Vector3 back_vec   = dir * head_part * arrow_length;
    const Vector3 side_vec   = right * horizontal_part * arrow_length * head_part;
    const Vector3 right_wing = end - back_vec + side_vec;
    const Vector3 left_wing  = end - back_vec - side_vec;

    // Create vertices
    Ogre::ColourValue colour_a = getColour(colour, alpha);
    int i = addLineVertex(start, colour_a);
    addLineVertex(end, colour_a);
    addLineVertex(right_wing, colour_a);
    addLineVertex(left_wing, colour_a);

    addLineIndices(i, i + 1);
    addLineIndices(i + 1, i + 2);
    addLineIndices(i + 1, i + 3);
}
...
void DebugDrawer::buildLine(const Ogre::Vector3& start,
                     ...
                     float alpha)
{
        int i = addLineVertex(start, getColour(colour, alpha));
        addLineVertex(end, getColour(colour, alpha));
...
void DebugDrawer::buildQuad(const Ogre::Vector3 *vertices,
                          ...
                          float alpha)
{
        int index = addLineVertex(vertices[0], getColour(colour, alpha));
        addLineVertex(vertices[1], getColour(colour, alpha));
        addLineVertex(vertices[2], getColour(colour, alpha));
        addLineVertex(vertices[3], getColour(colour, alpha));
 
        for (int i = 0; i < 4; ++i) addLineIndices(index + i, index + ((i + 1) % 4));
}

int DebugDrawer::createCircleSectorVertices(const Ogre::Vector3& centre,
                                    float radius,
                                    float angle_rads,
                                    const Ogre::Vector3& direction,
                                    int segmentsCount,
                                    const Ogre::ColourValue& colour,
                                    bool drawBoundingRadiuses,
                                    const Ogre::Vector3& normal,
                                    float alpha,
                                    bool filled)
{
    const int index = filled ? trianglesIndex : linesIndex;
    const float increment = angle_rads / (segmentsCount - 1);
    const float base_angle = -angle_rads * .5f;

    Ogre::Matrix3 to_world_space;
    to_world_space.FromAxes(direction, normal, direction.crossProduct(normal));

    Ogre::ColourValue colour_alpha = getColour(colour, alpha);
    for (int i = 0; i < segmentsCount; i++)
    {
        const float segment_angle = base_angle + i * increment;
        const Vector3 pos_2d(radius * Ogre::Math::Cos(segment_angle), 0.f, radius * Ogre::Math::Sin(segment_angle));
        const Vector3 pos_3d = centre + to_world_space * pos_2d;

        if (!filled)
            addLineVertex(pos_3d, colour_alpha);
        else
            addTriangleVertex(pos_3d, colour_alpha);
    }

    if (drawBoundingRadiuses)
        addLineVertex(centre, colour_alpha);
    else if (filled)
        addTriangleVertex(centre, colour_alpha);

    return index;
}

void DebugDrawer::buildCircleSector(const Ogre::Vector3& centre,
                                    float radius,
                                    float angle_rads,
                                    const Ogre::Vector3& direction,
                                    int segmentsCount,
                                    ...
                                    float alpha)
{
    const int index = createCircleSectorVertices(centre, radius, angle_rads,
        direction, segmentsCount, colour, drawBoundingRadiuses, normal, alpha);

    for (int i = 0; i < segmentsCount - 1; i++)
        addLineIndices(index + i, index + i + 1);

    if (drawBoundingRadiuses)
    {
        addLineIndices(index + segmentsCount, index);
        addLineIndices(index + segmentsCount, index + segmentsCount - 1);
    }
}

void DebugDrawer::buildFilledCircleSector(const Ogre::Vector3& centre,
                                          float radius,
                                          float angle_rads,
                                          const Ogre::Vector3& direction,
                                          int segmentsCount,
...
{
    const int index = createCircleSectorVertices(centre, radius, angle_rads,
        direction, segmentsCount, colour, false, normal, alpha, true);

    const int centre_index = index + segmentsCount;

    for (int i = 0; i < segmentsCount - 1; i++)
        addTriangleIndices(index + i + 1, index + i, centre_index);
}
                              ...
                              float alpha)
{
    ...
    addTriangleVertex(Ogre::Vector3(centre.x, centre.y + height / 2, centre.z), getColour(colour, alpha));
...
{
    int index = addLineVertex(vertices[0], getColour(colour, alpha));
    for (int i = 1; i < 8; ++i) addLineVertex(vertices[i], getColour(colour, alpha));
...
void DebugDrawer::buildFilledCuboid(const Ogre::Vector3 *vertices,
                                                          ...
                                                          float alpha)
{
    int index = addTriangleVertex(vertices[0], getColour(colour, alpha));
    for (int i = 1; i < 8; ++i) addTriangleVertex(vertices[i], getColour(colour, alpha));
...
{
    int index = addTriangleVertex(vertices[0], getColour(colour, alpha));
    addTriangleVertex(vertices[1], getColour(colour, alpha));
    addTriangleVertex(vertices[2], getColour(colour, alpha));
    addTriangleVertex(vertices[3], getColour(colour, alpha));
...
void DebugDrawer::drawCircleSector(const Ogre::Vector3& centre,
                                   float radius,
                                   float angle_rads,
                                   const Ogre::Vector3& direction,
                                   int segmentsCount,
                                   ...
                                   const Ogre::Vector3& normal)
{
    buildCircleSector(centre, radius, angle_rads, direction, segmentsCount, colour, drawBoundingRadiuses, normal);
    if (isFilled)
        buildFilledCircleSector(centre, radius, angle_rads, direction, segmentsCount, colour, normal);
}
                             ...
                             bool isFilled)
{
    ...
    if (isFilled)
    {
        ...
        trianglesIndex += icoSphere.addToVertices(&triangleVertices, centre, getColour(colour, fillAlpha), radius);
...
Ogre::ColourValue DebugDrawer::getColour( const Ogre::ColourValue& original, const float alpha ) const
{
    if (original.a != 1.f)
        return original;
    else
        return Ogre::ColourValue(original.r, original.g, original.b, alpha);
}
